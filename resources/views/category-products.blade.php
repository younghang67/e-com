...
@section('content')
            ...
            <ol class="list-none p-0 inline-flex items-center space-x-2">
                ...
                <li>
                    <svg class="w-3 h-3 text-gray-400" fill="currentColor" viewBox="0 0 320 512">
                        <path d="M285.5 273l-194 194c-9.4 9.4-24.6 9.4-33.9 0l-22.6-22.6c-9.4-9.4-9.4-24.6 0-34L184.5 256 34.1 105.6c-9.4-9.4-9.4-24.6 0-34l22.6-22.6c9.4-9.4 24.6-9.4 33.9 0l194 194c9.3 9.4 9.3 24.6-.1 34z"/>
                    </svg>
                </li>
                <li>
    ...
    <section class="container mx-auto px-4 py-6">
        ...
        <p class="text-center text-gray-500 mb-8 text-sm">Discover the best products in {{ strtolower($category->name) }} category</p>
    </section>
    ...
    <section class="container mx-auto px-4 flex flex-col md:flex-row gap-8">
        ...
        <aside class="md:w-1/4 w-full">
            <form method="GET" action="{{ route('category.products', $category->id) }}" class="space-y-6 bg-gray-100 p-6 rounded-lg shadow-sm">
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <input type="number" name="price_min" placeholder="Min" value="{{ request('price_min') }}" class="w-1/2 border-gray-300 px-3 py-2 rounded-md">
                        <input type="number" name="price_max" placeholder="Max" value="{{ request('price_max') }}" class="w-1/2 border-gray-300 px-3 py-2 rounded-md">
                    </div>
                </div>

                <!-- In Stock -->
                <div class="flex items-center">
                    <input type="checkbox" name="in_stock" value="1" {{ request('in_stock') ? 'checked' : '' }} class="rounded mr-2">
                ...
                @forelse($products as $product)
                    <a href="{{ route('product.detail', $product->id) }}" class="group">
                        <div class="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition">
                            <div class="relative">
                                <img src="{{ asset($product->main_image ?? 'img.png') }}" alt="{{ $product->name }}" class="w-full h-64 object-cover">
